...
function Init(){
    Loading_Image(0);
    sprites.push({sprite: new Sprite(1, new Vector2(5, 5)), checked: false});
    // sprites.push({sprite: new Sprite(2, new Vector2(15, 5)), checked: false});
    ...
    for(let index = 0; index < sprites.length; index++){
        let sprite = sprites[index].sprite;
        sprite_map[sprite.pos.y][sprite.pos.x] = index+1;
    }
...
function DrawMap(){
    ...
    for(let y = 0; y<HEIGHT; y++){
        for(let x = 0; x<WIDTH; x++){
            ...
            else if(sprite_map[y][x] !== undefined) map_ctx.fillStyle = "green";
...
function draw(){
    ...
    DrawGame();
    player.draw();
    for(let lay of player.Rays){
        map_ctx.beginPath();
        map_ctx.fillStyle = "crimson";
        //map_ctx.arc(lay.intersect.x, lay.intersect.y, PLAYER_RADIUS, 0, Math.PI * 2);
        map_ctx.closePath();
        map_ctx.fill();
    }
